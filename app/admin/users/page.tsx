...
export default function UserManagement() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await fetch("/admin/users", {
        ...
        body: JSON.stringify({ data: { page: 1, limit: 100, filters: {} } }),
      })
  ...
  const blockUser = async (userId: string) => {
    try {
      ...
      setUsers((prevUsers) => prevUsers.map((user) => (user.id === userId ? { ...user, isBlocked: true, isActive: false } : user)))
    } catch (err) {
  ...
  const unblockUser = async (userId: string) => {
    try {
      ...
      // Update user status in the local state
      setUsers((prevUsers) => prevUsers.map((user) => (user.id === userId ? { ...user, isBlocked: false, isActive: true } : user)))
    } catch (err) {
  ...
  return (
    <div className="flex min-h-screen">
      <AdminSidebar onUserSearch={setSearchQuery} />
              ...
              <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                <div>
                  ...
                  <CardDescription className="text-[#8892b0]">
                    Total {users.length} users, {users.filter((u) => u.isActive && !u.isBlocked).length} active
                  </CardDescription>
                </div>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredUsers.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="py-6 text-center text-[#8892b0]">
                      ...
                      filteredUsers.map((user) => (
                        <tr key={user.id} className="border-b border-[#1d3557]">
                          ...
                          <td className="py-4">
                            ...
                            >
                              {user.isBlocked ? "blocked" : user.isActive ? "active" : "pending"}
                            </Badge>
                          </td>
